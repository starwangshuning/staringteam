...
class Wechat
{
	...
	const API_URL_PREFIX_DATACUBE = 'https://api.weixin.qq.com';
...
//    }
	 ...
	 * @author wangshuning
	 * @date 2016-05-19
	 * @param string $scene_str json名值对
	 * @return array('ticket'=>'qrcode字串','expire_seconds'=>2592000,'url'=>'二维码图片解析后的地址')
	 */
	public function getQRCodeForever($scene_str){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
	...
	public function getDatacube($type,$subtype,$begin_date,$end_date=''){
	    ...
	    $result = $this->http_post(self::API_URL_PREFIX_DATACUBE.self::$DATACUBE_URL_ARR[$type][$subtype].'access_token='.$this->access_token,self::json_encode($data));

//		$debugpath = '/weitrade/'.wechatConfig::getProjectName().'/' . '/debugdatacube1.txt';
//		$u=self::API_URL_PREFIX.self::$DATACUBE_URL_ARR[$type][$subtype].'access_token='.$this->access_token;
//		file_put_contents($_SERVER['DOCUMENT_ROOT'].$debugpath,$u);
//		$debugpath = '/weitrade/'.wechatConfig::getProjectName().'/' . '/debugdatacube2.txt';
//		$uu=self::json_encode($data);
//		file_put_contents($_SERVER['DOCUMENT_ROOT'].$debugpath,$uu);
	    if ($result)
	    {
	        $json = json_decode($result,true);
	        if (!$json || !empty($json['errcode'])) {
